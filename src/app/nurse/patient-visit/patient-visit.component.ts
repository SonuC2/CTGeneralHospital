...
export class PatientVisitComponent implements OnInit {
  ...
  displayedColumsProcedure: string[] = [
    'proCode',
    'proDescription',
    'actions',
  ];
  ...
  medicationForm!:any;
  displayedColumsMedication: string[] = [
    'drugID',
    'drugName',
    'actions',
  ];
      ...
      this.schedulingService
        ...
        .subscribe((response) => {
          ...
          this.appointmentDetailsForm.patchValue({
            ...
            appointmentDate: moment( this.appointmentDetails.appointmentDate).format("DD-MM-YYYY"),
        ...
        console.log("Patietn id:" , this.appointmentDetails.patientId)
        this.patientService
        .getAllPatientDetails(this.appointmentDetails.patientId)
        .subscribe((patient) => {
          this.patient = patient;
          console.log('Patient details by id:', this.patient);
          const patientDetails = this.patient;
  ...
  loadDiagnosisDescription(diagnosis:Diagnosis, event:any){
    ...
    if(event.isUserInput){
      ...
      this.diagnosisService.getDiagnosisDescriptionByCode(diagnosis.diagnosisCode).subscribe(response =>{
        this.selectedDiagnosis =response;
        this.diagnosisForm.get('diagnosisDescription').setValue(response.diagnosisDescription)
      })
    }
  ...
  loadProcedureDescription(procedure:Procedure, event:any){
    if(event.isUserInput){
      ...
      this.procedureService.getProcedureDescriptionByCode(procedure.procedureCode).subscribe(response =>{
        this.selectedProcedure =response;
        this.procedureForm.get('procedureDescription').setValue(response.procedureDescription)
      })
    }
   ...
   loadMedicationDetails(medication:Medication, event:any){
    ...
    if(event.isUserInput){
      ...
      this.medicationService.getDrugDetailsByDrugId(medication.drugNo).subscribe(response =>{
  ...
  addPrescription(){
    ...
    this.patientVisitForm.get('patientVisit')?.get('patientVisitId').setValue(this.patientVisitDetails?.patientVisitId)
    ...
    this.prescriptionService.getAllPrescriptionByAppointment(this.appointmentDetails.appointmentId).subscribe(response =>{
      this.dataSourcePrescription.data =response;
      console.log("Prescription datasource", this.dataSourcePrescription.data);
      
    })
  }

  addPatientVisit(){
    this.patientVisitForm.get('appointment')?.get('appointmentId').setValue(this.appointmentDetails.appointmentId)
    this.patientVisitForm.get('patientId').setValue(this.appointmentDetails.patientId)
    this.patientVisitForm.get('patientName').setValue(this.appointmentDetails.patientName)
    this.patientVisitForm.get('visitStatus').setValue('Added')
    console.log("visit form", this.patientVisitForm.value)
    this.patientVisitService.submitPatientVisitDetails(this.patientVisitForm.value).subscribe();
  }
  ...
  onTabClick(event:any){
    ...
    if(this.userDetailsFromLogin.userRoleId.roleType === "Physician"){
      this.patientVisitService.getPatientVisitDetailsByPatientAndAppointmentId(this.appointmentDetails.patientId, this.appointmentDetails.appointmentId).subscribe(response =>{
  ...
  updateVisitDetails(){
    ...
    this.patientVisitService.updatePatientVisit(this.patientVisitDetails.patientVisitId, this.patientVisitDetails).subscribe();
